...
import { productGalleryPlaneData } from '../data/productGalleryData'

class Gallery {
  constructor(debug = null, container = null) {
    ...
    this.planeConfig = productGalleryPlaneData
  ...
  getViewportWidth() {
    return this.getContainerBounds().width
  }
  ...
  getPlaneLabelData(planeDefinition, index) {
    const fallback = {
      word: `Product ${String(index + 1).padStart(2, '0')}`,
      category: 'Industrial',
      description: '',
      color: '#F8FAF9',
      highlight: '#5CBF2A',
    ...
    return {
      ...
      highlight: label.highlight || fallback.highlight,
    }
  }
